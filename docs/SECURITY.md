...
There is no trusted-proxy address list, no hop count, and no configurable header name. `security.trustProxy` is the only switch, and the two header names are fixed.

The web BFF forwards `x-real-ip` but does not forward browser-supplied `x-forwarded-for`, `forwarded`, or `authorization` headers. Internal work such as cron and recovery is attributed as `internal`; platform turns use a platform identity rather than inventing an IP. Neither falls back to the last human IP seen.

Two things depend on this resolution: the login and SSO rate limiters, and usage attribution. The origin rollup behind `GET /usage/by-origin` carries client IP addresses across every account, so that route is administrator-only at the server, not merely hidden in the UI. Rows bearing an IP are collapsed into a single redacted bucket once they pass `runtime.limits.originIpRetentionDays`, which defaults to 30, and are deleted outright at the event retention horizon. A bucket is only as trustworthy as its weakest contribution, so one untrusted request marks the whole bucket untrusted.
...
Memory is private by default, but an administrator can configure a Project shared-memory pool for eligible Project members. The pool remains bounded by Project membership and is the deliberate exception to personal-memory isolation.
...
A conversation bound to a Sandbox workspace is confined regardless of who drives it, the instance operator included: the turn starts in the worktree and a shell command runs in a bubblewrap container exactly when its working directory lies inside an active workspace of the acting account: the worktree at `/workspace`, the account HOME at `/home/elowen`, a fresh `/tmp` per command, `/usr` read-only, an `/etc` allow-list, and no host application directory, systemd, `sudo`, `elowen` CLI or control-plane token. The worktree's `.git` is a stub, so repository metadata and the host-absolute gitdir pointer are never exposed; commits go through `SandboxCommit`. Files tools stay on the host behind the path guard. A child spawned from a bound conversation inherits the confinement; a `read_only` child has no writable scratch beyond that `/tmp` and returns its output as the delegation result. The binding is undone, without destroying anything, through `SandboxReleaseWorkspace`, the `/sandbox` picker or `POST /plugins/sandbox/api/workspaces/release`, and the release is refused while an execution lease is alive. A missed heartbeat does not free a live or unverifiable owner: expiry alone cannot prove that its writers stopped. Normal process and delegation completion release their leases explicitly, independently of the daemon lifetime. Stale reconciliation reclaims only provably dead or reused process identities. If an in-process holder abandons cleanup while its daemon remains alive, the lease conservatively keeps release, removal and HOME reset blocked; intervention requires establishing that the protected work has ended, not merely waiting for the heartbeat deadline.

A managed Project is a stronger boundary than a workspace and a different one. Its execution target is declared rather than inferred from a path, and it runs in its own persistent environment with its own workspace, home and data volumes, where each command executes as a transient unit. Every environment is a systemd-nspawn machine shifted into a uid range of its own, reaching root only through one pinned root-owned privileged helper, and confined with dropped capabilities, a seccomp policy, `DevicePolicy=closed` and read-only binds. Its root filesystem comes from a published artifact verified against a pinned sha256 digest, so no root filesystem is ever built on the host and there is no image store or registry in the trust path. The file, shell, browser, editor, LSP, MCP, and codebase surfaces reach it through the Sandbox control instead of the host filesystem, so a host path is not an address there; even a spilled tool result is written inside the guest and named by a guest path. Machine specifications are host-derived and frozen, so a caller-supplied mount list is not an execution capability, and `prepareExecution` offers no argument that asks for unconfined execution. Revoking an account's project assignment also cleans up its managed runtime access.

## Plugins, MCP, and secrets

Fresh installs enable the core plugin set `files`, `sandbox`, `terminal`, `askuser`, `runtime-context`, `subagent`, `elowen-docs`, `statusline`, `mcp`, and `changelog`. Actual access remains subject to manifest capabilities, per-account grants, per-turn policy, and owner checks.
...
- Plugin webhooks under `/hooks/*` are public at the daemon auth layer because the owning plugin must authenticate the external provider request. The body is capped at 1 MiB, an unknown mount answers 404 without revealing what exists, and a handler failure answers 500 with the detail kept daemon-side.
- Plugin WebSocket mounts under `/ws/plugins/<plugin>/<path>` are authenticated by a short-lived one-shot ticket rather than a bearer token, because the browser reaches the mount directly and carries no token. A plugin mints the ticket from an already-authenticated API request; it is held in memory, expires within five minutes at most, and is consumed on the attempt. An unusable ticket is refused with a plain 401 before the upgrade. A ticket binds to the account id the minting plugin supplies and nothing re-checks who asked for it, so minting a ticket for another account hands out that account's access.
