...
A managed Project is a stronger boundary than a workspace and a different one. Its execution target is declared rather than inferred from a path, and it runs in its own persistent environment with its own workspace, home and data volumes, where each command executes as a transient unit. A machine environment is shifted into a uid range of its own and reaches root only through one pinned privileged helper; a Podman environment created before that runtime stays rootless. The file, shell, browser, editor, LSP, MCP, and codebase surfaces reach it through the Sandbox control instead of the host filesystem, so a host path is not an address there; even a spilled tool result is written inside the guest and named by a guest path. Container specifications are host-derived and frozen, so a caller-supplied mount list is not an execution capability, and `prepareExecution` offers no argument that asks for unconfined execution. Revoking an account's project assignment also cleans up its managed runtime access.
