...
**Elowen today**: `collapseHistoricalImages()` (`src/brain/session/historyImageStripping.ts`) runs on **every** cold-cache turn (same TTL-based gate as F6/F7, raised to the provider's own maximum retention because this pass is destructive), collapsing every image block into a `[image omitted from history]` placeholder — proactively, not just at compaction time. It mutates the **live** context in place rather than an egress copy, and the rehydration path runs the same function over the stored rows, so what a respawn or a fork child replays is what the session was already sending. No latch: adjacent images collapse into one placeholder on both paths, and a message with no image is returned unchanged, which is what makes the pass idempotent.

**Gain from adopting CC's approach**: none — Elowen's mechanism is strictly more thorough. CC lets images accumulate in the live conversation indefinitely between compactions (each one re-serialized into every provider call, per the module comment on `collapseHistoricalImages` explaining exactly this cost); Elowen prevents that steady-state growth entirely.
