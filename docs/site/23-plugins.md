...
| `sites` | `SiteCreate`, `SitePublish`, `SiteRollback`, sharing, logs, immutable static releases, and command or PHP runtimes. | Sites use a Project source folder. Command and PHP runtimes are administrator-disabled by default, and public visibility requires an explicit confirmation in the Sites screen. | [Sites](sites-plugin) |
