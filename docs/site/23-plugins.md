...
| `sites` | Immutable static releases, managed Project proxy publications, previews, rollback and sharing. | Sites copies finished static output or forwards to an application already running in a managed Project. Public visibility requires explicit confirmation in the Sites screen. | [Sites](sites-plugin) |
