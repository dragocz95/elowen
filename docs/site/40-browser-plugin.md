...
The plugin has two modes. When no managed project is selected, the account gets its private browser, which requires a linked Elowen account in a private conversation. When a managed project is selected, the browser runs inside that project's environment instead, with the profile, cookies, and downloads shared as project data among project members.
...
| Tool | What it does |
| --- | --- |
| `BrowserOpen` | Opens a browser for the account, or inside the selected managed project, and returns the first page snapshot. |
...
- Unlinked senders, shared rooms, and delegated child agents cannot use the browser tools at all: the tools require a linked Elowen account in a private conversation, and a delegated child cannot borrow its parent's account. Project browsers require a managed project selection plus an acting account.
