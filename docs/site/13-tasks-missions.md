...
By default, `WorkflowStart` returns a handle and Elowen delivers the workflow summary when the DAG finishes. Set `background: false`, as an argument or in the file, to wait for the whole DAG and receive the node results as the tool result; an explicit argument overrides the file. On a surface that cannot deliver a later turn, the workflow waits instead so its summary is not lost. A blocking run can still be detached with **`Ctrl+B`** without cancelling it. Delivery is the only thing this choice changes: nodes keep running in dependency order either way, and a background workflow continues even if the conversation that started it is aborted.
