...
`GithubConnectionStatus, GithubRepositoryStatus, GithubListPullRequests, GithubGetPullRequest, GithubPullRequestChecks, GithubPublishBranch, GithubCreatePullRequest, GithubSubmitReview, GithubMergePullRequest` with device-auth and committed-branch publishing. Covers CC’s gh-via-Bash workflow with structured tools — but has **no subscription/wake-on-PR-event** equivalent.
