...
6. **Execution identity** — personal tools, memory, secrets, and managed-environment access are re-checked for the acting account at execution time.
...
## Projects, paths, and environments
...
The bundled `sandbox` plugin owns managed Project environments and prepares confined execution, and core consumes its typed `SandboxControl` live on every use; it does not read the plugin's tables directly. Terminal and delegated execution receive a prepared launch with its working directory, HOME, roots, confinement mode, and a durable execution lease.
...
A Project's execution target is declared in the shared wire contract as host or managed, and the Project DTO carries an `executionKind`. A managed target always names its stable registry identity; the target is never inferred from a filesystem path. A managed Project runs in its own persistent environment that survives across turns: a systemd-nspawn machine driven through one root-owned privileged helper, on a persistent environment disk filled from a published, content-addressed root filesystem artifact that the host downloads and verifies against a pinned sha256 digest. The catalogue of those artifacts is `plugins/sandbox/lib/rootfsCatalog.mjs` and the store that fetches and verifies them is `plugins/sandbox/lib/rootfsArtifacts.mjs`; nothing is built on the serving host. The project volume is mounted under the project's own name (`/kolin`, derived from the slug and refused when it would take over a directory the root filesystem already owns), beside a home volume and a data volume at `/data`; each execution runs inside as a transient systemd unit, and networking is either shared with the host with loopback denied or none. Machine specs are frozen and host-derived, and a caller-authored mount list is explicitly not an execution capability. Each machine is shifted into a uid range of its own and confined with dropped capabilities, a seccomp policy, `DevicePolicy=closed` and read-only binds. The Sandbox control's environment half covers provisioning, start, stop, restart, delete, snapshot, restore, limits, `migrate-disk`, the administrator-only `migrate-runtime`, logs, guest file operations including chunked writes, managed worktrees, and preview bindings. Files, shell, browser, editor, LSP, MCP, and codebase surfaces reach a managed project through that control instead of the host filesystem, and on a managed project even tool-result spill files are written into the guest and named by a guest path. `prepareExecution` takes a closed set of lease kinds and offers no way to request unconfined execution.

A host Project can be converted to a managed one in place: `POST /projects/:id/adopt` (admin-only) keeps the row, its members and its history, records the directory it came from in `adopted_path` and clears the host path in one statement. The directory itself moves into the project's own workspace volume on the first start of the environment — one atomic rename on the same filesystem, otherwise a verified copy that first establishes free space at the destination — and `{ undo: true }` on the same route reverses the row while the environment has not taken the directory yet.

A Project environment can also carry published transports. `projectPublicationBinding` establishes the same forwarder a preview uses as a PERSISTENT systemd unit named after the publication, on `<storage-root>/broker/pub-<publicationId>.sock`, and records it in the runtime table keyed by project and publication — never by an account, because the visitor it answers is nobody's account. That record is what the machine-start path and reconciliation use to re-establish the forwarder, which is why presence of the socket file is never read as liveness, and `projectPublicationRelease` stops the unit and removes the socket and the record. The broker directory is created by `checkedHostPath` with 0700 and owned by the daemon's account, so the daemon connects to a publication socket without any privileged helper. New Sites publications use only this managed Project transport: core has no host-Project publication fallback and Sites has no command, PHP, copied-build or per-Site runtime path. Existing legacy file releases remain a plugin data compatibility concern and do not create a second runtime.
...
Which system-prompt templates a session runs on is decided in one place, `src/brain/session/personaRoute.ts`. The interactive persona is composed from ordered PARTS with different owners: `elowen` is identity (who the agent is, memory, the control plane, permissions, voice), `elowen-harness` describes the tool surface and is the same for every model, and the work rules are one of `elowen-work` or `codex-work` — the second being OpenAI Codex's own gpt-5.6 template carried verbatim, selected when code mode is active because those models were trained against that exact text. A scheduled turn replaces the whole thing with the focused `scheduled` template, a shared room appends the platform overlay, and a plugin holding `mutates:['prompt']` may replace the resolved prompt for one session through the `brain.session.beforeSpawn` hook. The prompt reader has no include syntax and needs none: the resolver returns names, the spawner joins the renders. Every branch there moves a whole cached prompt prefix, so each one is narrow on purpose.

A code-mode session also differs on the WIRE, and both differences follow from the same predicate. It composes no tool search at all: search has nothing to activate when every tool is reached from inside a script, and a deferred tool is declared to the model by name only — which is exactly what the `exec` catalogue must not do, since the model writing the script needs the full signature. And `src/brain/session/codexDeveloperPlacement.ts` rewrites the outgoing body into the shape Codex sends for its `use_responses_lite` models: `instructions` emptied, the prompt spliced into `input` as a `developer` message, the tools spliced beside it as one `functions` namespace inside an `additional_tools` item, `parallel_tool_calls` off and `reasoning.context` set to `all_turns`. A call inside the default namespace comes back under its bare name, so dispatch is unaffected. It hooks the `before_provider_request` payload, which is the one seam that covers both the WebSocket and the SSE transport.
