...
- **Dashboard** uses `DashboardView` to show the finish-setup prompt when needed, the current activity tile, team presence, the recent activity feed, and the per-caller `/dash/recap` surface. The server-prefetched recap can contain continue actions, yesterday's sessions, and a daily digest; it is optional and never replaces the static dashboard when unavailable.
...
- the primary shell navigation uses the Studio column/rail on wide regions and a drawer when space is constrained;
- configuration decks use a section sidebar from tablet width upward and a single-line, touch-scrollable secondary section nav on phones;
- register pages keep horizontal section tabs at every width;
- a left, right, top, or bottom advisor dock changes the available region and can mirror or stack navigation;
