...
Settings and Account are decks, and each is ONE row of the sidebar: the column opens a deck, and both decks carry their own section navigation on every surface that draws them (`SettingsNavigation.tsx`, `AccountNavigation.tsx` — a secondary column beside the content, one line of tabs above it on a phone, both derived from the section registries). The sidebar used to declare their sections as sub-items, including every plugin-contributed one, which put the longest lists in the product inside the column meant to reach anywhere. Active-route resolution (`web/components/shell/useSidebarRoute.ts`) scores the path prefix plus a matching `?cat=` and a matching hash, so `/settings?cat=models` beats plain `/settings`. Only `cat` is treated as an addressing parameter.
...
- **Projects** displays registered filesystem roots and the daemon's read-only Git snapshot. Administrators can create, edit, remove, and assign Projects; members can only use Projects granted to them. Enabled plugins can add project panels, such as managed Project environments or a GitHub repository mapping.
...
The bundled `sandbox` plugin contributes a Project panel labelled `Environments` and an administrator User-detail panel at `Development environment`. It provides managed Project environment status and lifecycle controls, the persistent account HOME, process leases, and confinement preparation. The optional GitHub plugin contributes account and Project panels for an account's GitHub connection, Project repository mappings, pull requests, checks, reviews, branch publication, and explicitly confirmed merges. GitHub publication requires both a verified repository mapping and a committed branch.
