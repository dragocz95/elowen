...
import type { BrainLimits, RuntimeConfig, RuntimeLimits, MemoryRetentionConfig } from '../../lib/types';
...
/** Settings → Brain: runtime policy plus the extracted provider account section. */
export function BrainSection({ onSaveState }: { onSaveState?: (section: string, status: SaveStatus, retry?: () => void) => void }) {
  ...
  if (!config) return <><LoadingState /><BrainProvidersSection config={config} /></>;
  return (
    <>
