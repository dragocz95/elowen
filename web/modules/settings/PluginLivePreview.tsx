...
function DiscordPreview({ detail, values }: { detail: PluginDetail; values: Record<string, unknown> }) {
  ...
  const outputPreview = output === 'hidden' ? null : output === 'tail' ? (
    <pre className="mt-1.5 whitespace-pre-wrap rounded-md bg-background/70 px-2 py-1.5 text-[10px] leading-relaxed text-foreground">{'$ npm test\n✓ 42/42\n…'}</pre>
  ) : <span className="mt-1 block truncate text-[10px] text-foreground">✓ 42/42</span>;
  const bubble = (name: string, status: string, withOutput = false) => (
    <div data-testid="discord-tool-bubble" className={previewBox}>
      <div className="flex items-center gap-2 font-mono text-[11px] text-muted-foreground"><Wrench size={12} className="text-primary" aria-hidden /><span>{name}</span><span className="ml-auto">{status}</span></div>
      {withOutput ? outputPreview : null}
    </div>
  );
  const toolPanel = tools === 'off' ? (
    <div className={previewBox}>
      <div className="mb-1 flex items-center gap-2 text-xs font-medium text-foreground"><Wrench size={13} className="text-primary" aria-hidden />{t.pluginDetail.previewToolActivity}</div>
      <span className="text-xs text-muted-foreground">{t.pluginDetail.previewHidden}</span>
    </div>
  ) : perTool ? (
    ...
    <div data-testid="discord-tool-bubble" className={previewBox}>
      <div className="mb-2 flex items-center gap-2 text-xs font-medium text-foreground"><Wrench size={13} className="text-primary" aria-hidden />{t.pluginDetail.previewToolActivity}</div>
  ...
  return (
    <div className="@container">
      <div data-testid="discord-preview-layout" className="grid grid-cols-1 gap-2.5 @lg:grid-cols-[minmax(0,1.35fr)_minmax(0,.65fr)] @lg:items-stretch">
        {toolPanel}
        <div className={`${previewBox} text-xs leading-relaxed text-foreground`}>
...
function WhatsAppPreview({ detail, values }: { detail: PluginDetail; values: Record<string, unknown> }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="ml-5 rounded-xl rounded-tr-sm bg-success/10 px-3 py-2.5 text-xs leading-relaxed text-foreground">
        {streaming ? t.pluginDetail.previewStreamingAnswer : t.pluginDetail.previewFinalAnswer}
      </div>
      <div className="flex items-center justify-end gap-2"><span className="text-tiny text-muted-foreground">{t.pluginDetail.previewReasoning}</span><OnOff on={reasoning} /></div>
    </div>
  );
}
...
function CronPreview({ detail, values }: { detail: PluginDetail; values: Record<string, unknown> }) {
  ...
  return (
    <div className="rounded-lg border border-border/70 bg-foreground/[0.02] p-3">
      <div className="mb-3 flex items-center gap-2 text-sm font-medium text-foreground"><Clock3 size={15} className="text-primary" aria-hidden />{t.pluginDetail.previewScheduler}</div>
      <dl className="grid grid-cols-2 gap-2 text-xs">
        <dt className="text-muted-foreground">{t.pluginDetail.previewCheckEvery}</dt><dd className="text-right font-mono text-foreground">{tick}s</dd>
        <dt className="text-muted-foreground">{t.pluginDetail.previewAttempts}</dt><dd className="text-right font-mono text-foreground">{attempts}</dd>
      </dl>
    </div>
  );
}
...
function TerminalPreview({ detail, values }: { detail: PluginDetail; values: Record<string, unknown> }) {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-background font-mono text-[11px]">
      <div className="flex items-center gap-2 border-b border-border px-3 py-2 text-muted-foreground"><TerminalSquare size={13} aria-hidden />Bash</div>
      <div className="space-y-1 p-3 text-foreground"><div>$ npm test</div><div className="text-success">✓ {t.pluginDetail.previewDone}</div></div>
      <div className="border-t border-border px-3 py-2 text-muted-foreground">{Math.round(cap / 1000)} KB · {timeout}s</div>
    </div>
  );
}
...
function GenericPreview({ detail, values, fieldLabel }: { detail: PluginDetail; values: Record<string, unknown>; fieldLabel: (field: PluginConfigField) => string }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-3"><PluginIcon name={detail.name} hasIcon={detail.hasIcon} size={38} /><div><div className="text-sm font-medium text-foreground">{detail.name}</div><div className="text-xs text-muted-foreground">{detail.enabled ? t.pluginDetail.statusEnabled : t.pluginDetail.statusDisabled}</div></div></div>
      {fields.length ? <dl className="flex flex-col gap-2 border-t border-border pt-3">{fields.map((field) => {
        ...
        return <div key={field.key} className="flex items-center justify-between gap-3 text-xs"><dt className="truncate text-muted-foreground">{fieldLabel(field)}</dt><dd className="max-w-[50%] truncate font-mono text-foreground">{typeof raw === 'boolean' ? (raw ? t.settings.on : t.settings.off) : String(raw ?? '—')}</dd></div>;
      })}</dl> : <p className="text-xs text-muted-foreground">{t.pluginDetail.previewNoConfig}</p>}
    </div>
  );
}
...
export function PluginLivePreview({ detail, values, fieldLabel }: {
  ...
  else if (name.includes('personality')) body = <div className="flex items-center gap-2 rounded-lg border border-border/70 bg-foreground/[0.02] p-3 text-sm text-foreground"><Bot size={15} className="text-primary" aria-hidden />{t.pluginDetail.previewPersonality}</div>;
  else body = <GenericPreview detail={detail} values={values} fieldLabel={fieldLabel} />;
  return (
    <section aria-label={t.pluginDetail.livePreview} className="rounded-[var(--radius-lg)] border border-foreground/[0.075] bg-foreground/[0.012] p-4">
      <header className="mb-4 flex items-center justify-between gap-2"><span className="flex items-center gap-2 text-sm font-medium text-foreground"><MessageCircle size={15} className="text-primary" aria-hidden />{t.pluginDetail.livePreview}</span><span className="inline-flex items-center gap-1 font-mono text-[9px] uppercase tracking-[0.08em] text-success"><Check size={11} aria-hidden />{t.pluginDetail.previewLive}</span></header>
      {body}
    </section>
  );
}
