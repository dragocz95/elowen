...
export function ElowenPresence({ state, compact = false, label }: {
  ...
  return (
    <div className={`elowen-presence elowen-presence--${state}${compact ? ' elowen-presence--compact' : ''}`} role="img" aria-label={label}>
      ...
      <style>{`
        .elowen-presence { --presence-hot: var(--color-primary); --presence-warm: var(--color-primary-hot); position: relative; display: grid; place-items: center; width: min(100%, 25rem); aspect-ratio: 1; isolation: isolate; }
        .elowen-presence--compact { width: 8rem; }
        .elowen-presence__mascot { position: relative; z-index: 5; width: 49%; height: auto; user-select: none; filter: drop-shadow(0 1rem 2rem rgb(var(--primary-rgb) / .16)); animation: elowen-breathe 4.8s ease-in-out infinite; }
        .elowen-presence__aura { position: absolute; inset: 19%; z-index: -2; border-radius: 999px; background: radial-gradient(circle, rgb(var(--primary-rgb) / .2), rgb(var(--primary-rgb) / .06) 42%, transparent 72%); filter: blur(18px); animation: elowen-aura 4.8s ease-in-out infinite; }
        .elowen-presence__orbit { position: absolute; z-index: -1; width: 76%; height: 36%; border: 1px solid rgb(var(--primary-rgb) / .28); border-radius: 50%; transform: rotate(-12deg); animation: elowen-orbit 13s linear infinite; }
        .elowen-presence__orbit::after { content: ''; position: absolute; left: 12%; top: 12%; width: .42rem; height: .42rem; border-radius: 999px; background: var(--presence-hot); box-shadow: 0 0 1rem rgb(var(--primary-rgb) / .8); }
        ...
        .elowen-presence__ground { position: absolute; bottom: 20%; width: 46%; height: 8%; border-radius: 50%; background: radial-gradient(ellipse, rgb(var(--primary-rgb) / .32), transparent 68%); filter: blur(8px); }
        ...
        .elowen-presence--needs_input .elowen-presence__aura { background: radial-gradient(circle, color-mix(in srgb, var(--color-warning) 28%, transparent), rgb(var(--primary-rgb) / .08) 48%, transparent 72%); animation-duration: 1.5s; }
        .elowen-presence--needs_input .elowen-presence__orbit { border-color: color-mix(in srgb, var(--color-warning) 45%, transparent); }
        .elowen-presence--success .elowen-presence__aura { background: radial-gradient(circle, color-mix(in srgb, var(--color-success) 18%, transparent), rgb(var(--primary-rgb) / .08) 46%, transparent 72%); }
