...
type PresenceState = 'offline' | 'idle' | 'thinking' | 'working' | 'needs_input' | 'success' | 'error';
 ...
 *  `success` and `error` remain in the union because the dictionary names them, and because the
 *  signal that would produce them — a parked question, a failed turn — is
...
export function usePresence(): Presence {
  ...
  const pulseKnown = pulse.data !== undefined && !pulse.isError;
  ...
  return {
    ...
    // beside "Offline" would be two contradictory claims drawn from the same screen. Loading and failed
    // pulse requests stay unknown instead of fabricating an idle zero.
    state: offline ? 'offline' : working.length > 0 ? 'working' : 'idle',
    primary: offline || !pulseKnown ? undefined : working[0],
    activeCount: offline || !pulseKnown ? undefined : working.length,
  };
}
