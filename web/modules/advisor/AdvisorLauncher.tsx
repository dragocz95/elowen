...
export function AdvisorLauncher({ onOpen }: { onOpen: () => void }) {
  ...
  return (
    <button
      ...
      className="advisor-launcher overlay-fab group flex h-14 w-14 items-center justify-center rounded-2xl border border-primary/25 bg-surface/90 shadow-[0_14px_40px_rgba(0,0,0,0.45)] backdrop-blur-xl transition-[transform,border-color,box-shadow] duration-200 hover:-translate-y-1 hover:border-primary/50 hover:shadow-[0_18px_48px_color-mix(in_srgb,var(--color-primary)_18%,transparent)] active:translate-y-0 active:scale-95"
    >
      <span className="advisor-launcher__ambient ambient-pulse absolute inset-1 animate-pulse rounded-[0.8rem] bg-primary/10 opacity-60" aria-hidden />
