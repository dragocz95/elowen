...
export function AdvisorPanel({ dock }: { dock: UseDockState }) {
  ...
  const panel = <div className={`advisor-panel flex shrink-0 flex-col overflow-hidden border-border bg-surface ${horizontal ? 'h-full' : 'w-full'}`} style={horizontal ? { width: `min(${state.width}px, 100vw)` } : { height: `min(${state.height}px, 85dvh)` }}>
