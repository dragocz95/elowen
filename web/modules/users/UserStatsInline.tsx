...
function Chip({ icon: Icon, label, children }: { icon: LucideIcon; label: string; children: React.ReactNode }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-md border border-border bg-muted px-2 py-1 text-xs text-foreground" title={label}>
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      <Chip icon={Brain} label={t.users.statMemories}>
        {d.memoryCount > 0 ? <span className="font-mono tabular-nums">{d.memoryCount}</span> : <span className="italic text-muted-foreground">{t.users.noMemories}</span>}
      </Chip>
      <Chip icon={MessagesSquare} label={t.users.statSessions}>
        {d.sessionCount > 0 ? <span className="font-mono tabular-nums">{d.sessionCount}</span> : <span className="italic text-muted-foreground">{t.users.noSessions}</span>}
      </Chip>
      <span
      ...
      >
        {d.topModel
          ...
          : <span className="italic text-muted-foreground">{t.users.noModel}</span>}
      </span>
    </div>
  );
}
