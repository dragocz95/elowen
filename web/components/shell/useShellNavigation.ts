...
export function useShellNavigation(): { worlds: NavEntry[]; allWorlds: NavEntry[]; layout: NavLayout; layoutReady: boolean } {
  ...
  // SETTINGS AND ACCOUNT ARE ONE DESTINATION EACH, not a disclosure over their sections.
  //
  // Each deck is a set of sections addressed by `?cat=`, and for a while the sidebar listed all of them
  // inline. That put the longest lists in the product — every settings category, every account section,
  // plus whatever the installed plugins contribute — inside the column that is supposed to get you
  // ANYWHERE, so the two most configuration-heavy corners of the app crowded out everything else and the
  // menu grew and shrank as plugins were enabled.
  //
  // Both decks already carry their own section navigation on the page: a searchable rail beside the
  // content where there is width for it, a strip above it on a phone. That navigation is where a section
  // belongs — it is next to the thing it changes, it can be searched, and it does not cost the top-level
  // menu its shape. The sidebar rows open the deck, and the deck takes it from there. Deep links and the
  // command palette still address every section directly; the palette builds its own index from the same
  // section lists rather than from these entries.
