...
export function UsageBadge({ usage }: { usage: TokenUsage }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 font-mono text-[11px]" title={tip}>
      ...
      <Pill icon={ArrowUpFromLine} label={t.usage.output} display={formatTokens(usage.output)} className="border-destructive/30 text-destructive" />
      {hasCost ? <Pill icon={Coins} label={t.usage.cost} display={costLabel} className={estimated ? 'border-border text-muted-foreground' : 'border-success/30 text-success'} /> : null}
    </span>
  );
}
