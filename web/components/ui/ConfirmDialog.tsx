...
import type { ConfirmDialogProps } from 'elowen-plugin-ui-kit';
...
export type { ConfirmDialogProps } from 'elowen-plugin-ui-kit';
...
function OpenConfirmDialog({
  ...
  const dismissBlocked = disabled || isPending;
  const confirmBlocked = dismissBlocked || confirmDisabled;
  const shownError = error ?? internalError;
  const close = () => { if (!dismissBlocked) onClose(); };

  const confirm = () => {
    if (confirmBlocked || confirmingRef.current) return;
    ...
    <AlertDialog open onOpenChange={(next) => { if (!next) close(); }}>
      <DialogOverlay ref={overlayRef} presentation="center">
        <AlertDialogContent
          ...
          data-elowen-modal
          onEscapeKeyDown={(event) => { if (dismissBlocked) event.preventDefault(); }}
        ...
        >
          <DialogHeader title={title} titleId={titleId} icon={AlertTriangle} closeLabel={t.common.close} closeDisabled={dismissBlocked} onClose={close} />
          <ModalBody>
          ...
          <ModalFooter>
            <Button ref={cancelRef} variant="ghost" onClick={close} disabled={dismissBlocked}>{t.common.cancel}</Button>
            <Button variant={confirmVariant} onClick={confirm} disabled={confirmBlocked}>
