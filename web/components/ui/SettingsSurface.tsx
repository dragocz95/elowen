...
export function SettingsGroup({ title, description, icon: Icon, actions, tone = 'default', density = 'comfortable', columns = 1, collapsible = false, defaultOpen, open, onOpenChange, children, className = '' }: {
  title?: string;
  description?: string;
  icon?: LucideIcon;
  actions?: ReactNode;
        ...
        <header className="settings-group__header">
          <CollapsibleTrigger asChild>
            <button
              ...
              // The native button already answers Enter/Space with a click in the browser, but jsdom
              // (and anything that suppresses the default action) does not, so the fold owns its keys
              // explicitly — and `preventDefault` keeps the browser's own click from firing a second
              // toggle behind the first.
              onKeyDown={(event) => {
