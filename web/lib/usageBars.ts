...
export interface UsageSummary {
  ...
  modelsUsed: number;
  /** Duration-weighted effective output speed across every row that measured one ('—' when none did).
   *  Legacy post-header samples remain unknown. */
...
export function buildUsageSummary(data: ModelUsage[] | undefined): UsageSummary {
  ...
  const rows: UsageRow[] = items
    .map((m) => ({
      ...
      speedLabel: formatSpeed(m.usage.effectiveTps),
  ...
  // Average speed is duration-weighted over effective samples only. Recover each row's request seconds
  // from its measured output over its rate; legacy post-header samples stay unknown.
  const avg = weightedSpeedFromPairs(items.map((m) => ({
