/** MIRROR of src/shared/chatPresentation.ts — kept byte-identical below the header.
 *
 * The web cannot import runtime code from src/: Turbopack resolves module paths relative to the web
 * root and fails the production build on anything outside it, with or without a `.js` suffix. The
 * wireContract import in lib/types.ts is the exception only because it is TYPE-ONLY and the bundler
 * erases it before resolution ever happens. tsc and vitest both resolve it happily, so this breaks
 * ONLY in `next build` — which is why the duplication is enforced by a parity test rather than by
 * discipline. Edit the source file, then copy it here; the test fails the moment they differ.
 */
