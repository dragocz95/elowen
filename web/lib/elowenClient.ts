...
export const elowenClient = {
  ...
  myTerminalSettings: () => req<TerminalSettingsSnapshot>('/auth/me/terminal-settings'),
  saveMyTerminalSettings: (patch: Partial<TerminalSettings>, expectedRevision?: number) => req<TerminalSettingsSnapshot>('/auth/me/terminal-settings', json({ ...patch, ...(expectedRevision === undefined ? {} : { expectedRevision }) }, 'PATCH')),
  ...
  installPlugin: (name: string, enable = true, acknowledgeGrants?: string[]) =>
    req<PluginInfo & { pending?: boolean }>(`/plugins/marketplace/${encodeURIComponent(name)}/install`, json({ enable, ...(acknowledgeGrants ? { acknowledgeGrants } : {}) })),
  ...
  sessionTasks: (session: string) => req<{ tasks: SessionTask[] }>(`/plugins/todo/api/tasks?session=${encodeURIComponent(session)}`),
  ...
  updateSessionTask: (session: string, taskId: string, patch: SessionTask['status'] | SessionTaskPatch) =>
    req<{ task: SessionTask; tasks: SessionTask[] }>(
  ...
  brainCommand: (name: string, session?: string, argument?: string) => req<{ ok?: boolean; message?: string; error?: string; data?: { goal?: BrainGoal | null } }>('/brain/command', json({ name, ...(argument ? { argument } : {}), ...(session ? { session } : {}) })),
  ...
  /** Rank the palette's own index against a query by embedding similarity — the semantic layer BEHIND the
   *  lexical filter, so it is only ever asked when the literal one came up thin. A 503 means this instance
   *  has no embedding provider (they need an API key, which an OAuth-only install does not have); the
   *  caller is expected to fall silent rather than report it. `signal` cancels a superseded keystroke. */
  searchRank: (query: string, candidates: SearchRankCandidate[], signal?: AbortSignal) =>
    req<SearchRankResult>('/search/rank', { ...json({ query, candidates }), ...(signal ? { signal } : {}) }),
  /** Ask the workspace's cheap model which of the palette's rows answer a query that matched nothing at
   *  all. One deliberate click, never a keystroke. Answers ids from the list that was sent — an id the
   *  model invented is dropped daemon-side, so the result is always resolvable against the local index. */
  searchAsk: (query: string, candidates: SearchAskCandidate[], signal?: AbortSignal) =>
    req<SearchAskResult>('/search/ask', { ...json({ query, candidates }), ...(signal ? { signal } : {}) }),
  ...
  /** Per-provider code-mode state: membership is the capability answer and `enabled` is the operator
   *  switch. Unlike hosted search there is no per-model verdict, because code mode has no model gate. */
  brainCodeModeStatus: () => req<{ providers: { providerId: string; enabled: boolean }[] }>('/brain/providers/code-mode/status'),
  ...
  /** The personalized dashboard recap (per-caller): continue pills, yesterday, the daily digest. */
  dashRecap: () => req<DashRecap>('/dash/recap'),
  /** Admin: drop the caller's digest row for today so the next GET regenerates (prompt/model tuning). */
  dashRecapRegenerate: () => req<{ ok: boolean }>('/dash/recap/regenerate', { method: 'POST' }),
  ...
  /** The same authorized membership, asked for the bounded identity a member row renders. Readable by an
   *  administrator, and for a managed project by its own members. */
  projectMemberProfiles: (id: number) => req<ProjectMemberView[]>(`/projects/${id}/users?view=profiles`),
  ...
  /** A managed project's environment state, read through the Sandbox plugin's read-only overview. That
   *  GET can never provision, so a surface that only needs to know whether the environment is running
   *  asks here instead of touching a file route, which would start one just by being called. */
  projectEnvironmentState: (id: number) =>
    req<ProjectEnvironmentOverview>(`/plugins/sandbox/api/projects/${id}/environment`),
