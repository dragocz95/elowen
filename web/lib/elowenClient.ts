...
export const elowenClient = {
  ...
  /** The caller's OWN per-plugin values. There is no user parameter by design — the daemon reads the
   *  session, so this endpoint can only ever return the signed-in account's own data. */
  myPluginConfigs: () => req<PluginUserConfig[]>('/plugins/user-config'),
  saveMyPluginConfig: (name: string, values: Record<string, unknown>) => req<PluginUserConfig>(`/plugins/${encodeURIComponent(name)}/user-config`, json({ values }, 'PATCH')),
