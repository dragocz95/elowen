...
export interface ElowenConfig {
  ...
  brain?: { providers: BrainProvider[]; agentName?: string; maxSteps?: number; modelContextWindows?: Record<string, number>; modelMaxTokens?: Record<string, number>; limits?: BrainLimits; forkParentContext?: boolean; hiddenOauth?: string[] };
 ...
 *  `parentSessionId` is the conversation that DELEGATED this one, so the register can nest a sub-agent
 *  under its actual parent. It may name a row that is not in the same response (deleting a parent detaches
 *  its children instead of cascading): such a session is rendered as a root. Optional so a payload from an
 *  older daemon still types; absent reads exactly like a root. */
export interface ManagedSession { id: string; title: string; provider?: string; model: string; updated_at: string; running: boolean; active: boolean; kind: 'conversation' | 'channel'; tokens: number; platform: string | null; direct: boolean; ownerId: number; ownerLabel: string; parentSessionId?: string | null; lastWriterId: number | null; lastWriterLabel: string | null }
...
export interface ConfigPatch {
  ...
  brain?: { providers?: (Omit<BrainProvider, 'apiKeySet'> & { apiKey?: string })[]; agentName?: string; maxSteps?: number; modelContextWindows?: Record<string, number>; modelMaxTokens?: Record<string, number>; limits?: Partial<BrainLimits>; forkParentContext?: boolean; hiddenOauth?: string[] };
...
interface PluginCapabilities {
  hooks?: string[];
  mutates?: ('prompt' | 'turnContext' | 'tools' | 'events' | 'workflow-dag')[];
  reads?: string[];
  network?: boolean;
}
...
export interface CronJob {
  ...
  enabled?: boolean;
  /** The conversation this recurring job is ORGANIZED under — grouping only. It never affects the job's
   *  execution context, model, privileges, schedule, origin binding or delivery destination.
   *
   *  Omitting the field on a save PRESERVES whatever is stored, including a target that has since become
   *  unavailable; that is how an older editor round-trips a job it knows nothing about. Sending a blank
   *  value is refused rather than treated as "detach". The server also keeps an immutable identity for the
   *  same target so the association survives a session-id change; that value is server-only and is never
   *  part of this payload, so a client can neither read nor forge it. */
 ...
 *  response of GET /brain/conversation-links. Organization only: opening the link opens the job's existing
 *  editor and changes nothing about when it runs, which model it uses or where its result is delivered.
 *
 *  `href` is generated by the daemon from the job id, never contributed by the plugin, so a link row can
 *  only ever point at the cron editor on this same origin. `scope` distinguishes a personal job from an
 *  instance one; the daemon has already applied job visibility, so every row here is one the caller may
 *  see. Prompts, schedules and results are deliberately absent — navigation never shows them. */
export interface ConversationJobLink {
  jobId: string;
 ...
 *  conversation list, `all` by the admin register and requires an administrator.
 *
 *  The three states are NOT interchangeable: `available` means the job list was read and `links` is
 *  complete for that scope; `unavailable` means there is nothing to ask (the cron plugin is disabled,
 *  uninstalled, too old to answer, or not granted to this account) and no branch should appear at all;
 *  `error` means the read genuinely failed and the caller must say so rather than render "no jobs". */
