export const en = {
  ...
  brain: {
    ...
    runtime: {
      ...
      memoryDuplicatePerMilleHint: "The similarity at which a new memory counts as a rewording of one already stored. Automatic memory writing then overwrites the existing memory instead of adding a second one, while a memory the assistant saves itself is always kept and merely reported as similar. Lower = fewer duplicates, but a growing risk of overwriting a memory that was actually about something else, which is why this is deliberately stricter than the recall-side threshold. The value belongs to the embedding model in use AND to how large the store has grown, since long entries written in a consistent style resemble one another even when they describe different things: re-check it after changing the model or after the store grows substantially. Range 0.50-0.98, default 0.93.",
