...
const cfg = (agentName: string) => ({ brain: { agentName } });
...
describe('resolveBrand', () => {
  ...
  it('an explicit configured agentName wins over the theme brand', () => {
    const brand = resolveBrand(cfg('Jarvis'), { agentName: 'Acme Bot', productName: 'Acme' }, 'acme');
  ...
  it('the stored default agentName does not block the theme', () => {
    const brand = resolveBrand(cfg(DEFAULT_AGENT_NAME), { agentName: 'Acme Bot' }, 'acme');
    expect(brand.agentName).toBe('Acme Bot');
  });

  it('a one-name theme brands both agent and product', () => {
    const brand = resolveBrand(cfg('Elowen'), { agentName: 'Acme Bot' }, 'acme');
