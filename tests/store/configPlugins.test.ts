...
describe('ConfigStore plugins', () => {
  it('defaults to the safe fresh-install tool set', () => {
    ...
    expect(cs.get().plugins.enabled).toEqual(['files', 'terminal', 'askuser', 'runtime-context', 'skills', 'subagent', 'elowen-docs', 'cronjob', 'security-scan', 'statusline', 'codebase', 'mcp', 'agents', 'lsp', 'editor']);
  });
