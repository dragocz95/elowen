...
describe('conversation target projection', () => {
  ...
  it('preserves the owner-only reply read beside organizational metadata', () => {
    store.createSession({ id: 'brain-2', userId: 2, model: 'm' });
    store.appendMessage({
      id: 'reply', sessionId: 'brain-2', parentId: null, role: 'assistant',
      content: { role: 'assistant', content: [{ type: 'text', text: 'Private reply' }] },
    });

    expect(targets.lastAssistantText('brain-2', 2)?.text).toBe('Private reply');
    expect(targets.lastAssistantText('brain-2', 1)).toBeNull();
    expect(targets.lastAssistantText('brain-2', 3)).toBeNull();
    expect(targets.lastAssistantText('unknown', 2)).toBeNull();
    expect(targets.resolve({ actorUserId: 1, sessionId: 'brain-2' })?.id).toBe('brain-2');
  });
