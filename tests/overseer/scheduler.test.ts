...
function setup(now: number) {
  ...
  const scheduler = new Scheduler({ tasks, spawn, bus: new EventBus(), project: { id: 1, path: '/o' }, fallback: { program: 'claude-code', model: 'sonnet' }, nameAgent: () => 'Nova', clock: new FakeClock(now) });
