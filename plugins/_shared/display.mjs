// Presentation-policy resolution shared by every platform adapter (`/display`): how tool activity, the
// answer, tool output and per-tool messages are rendered for one conversation. New enum settings win; the
// old streaming booleans remain a read-only fallback so existing installs keep their behaviour until the
// operator saves the new fields. A conversation may override any axis independently; absent axes inherit
...
/** Resolve the presentation policy for one conversation from the plugin's global config plus that
 *  conversation's `/display` overrides. */
