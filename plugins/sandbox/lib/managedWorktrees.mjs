...
path=$1; branch=$2; base=$3
g() { git -c core.hooksPath=/dev/null -c core.fsmonitor=false "$@"; }
if [ -e "$path/.git" ]; then
 test "$(g -C "$path" symbolic-ref --short HEAD)" = "$branch"
 test "$(g -C "$path" rev-parse --path-format=absolute --git-common-dir)" = /workspace/.git
elif g -C /workspace show-ref --verify --quiet "refs/heads/$branch"; then
 test "$(g -C /workspace rev-parse "refs/heads/$branch")" = "$base"
 g -C /workspace worktree add "$path" "$branch"
else
 g -C /workspace worktree add -b "$branch" "$path" "$base"
fi
`;
...
if [ -e "$path" ]; then
 ...
 if ! g -C /workspace show-ref --verify --quiet "refs/heads/$branch"; then exit 0; fi
 head=$(g -C /workspace rev-parse "refs/heads/$branch")
fi
test "$(g -C /workspace rev-list --count "$base..$head")" = 0 || fail 'Preserve worktree commits before removing it'
test "$(g -C /workspace rev-list --count "$head" --not --exclude="refs/heads/$branch" --all)" = 0 || fail 'Worktree branch retains otherwise unreachable commits'
if [ -e "$path" ]; then g -C /workspace worktree remove "$path"; fi
g -C /workspace update-ref -d "refs/heads/$branch" "$head"
`;

export async function manageWorktrees({ db, runGuest, row, userId, action }) {
  ...
  return await withRepoLease(db, `managed-worktrees:${projectId}`, async () => {
    if (action.kind === 'create') {
      ...
      if (!record) {
        const resolved = await runGuest(row, userId, ['/usr/bin/git', '-C', '/workspace', 'rev-parse', '--verify', '--end-of-options', `${action.baseRef}^{commit}`], { kind: 'worktrees' });
      ...
      const result = await runGuest(row, userId, ['/bin/bash', '-s', '--', record.path, record.branch, record.base_commit], { kind: 'worktrees', input: createScript });
      ...
      const result = await runGuest(row, userId, ['/bin/bash', '-s', '--', record.path, record.branch, record.base_commit], { kind: 'worktrees', input: removeScript });
