...
export function registerWorkflow(ctx, getRun, { resolveDelegateTools, principalOf, dependencyContextChunks }) {
  ...
  ctx.registerTool(defineTool({
    ...
    parameters: Type.Object({
      nodesFile: Type.String({ description: `Path to the JSON workflow definition. Create it with Write first — inside the project itself (for example /workspace/workflow.json) in a managed project, otherwise under ${workflowDir} for a one-off run if your session may write there, else inside an accessible repository. Pass either a node array or { title?, fork?, nodes, background? }.` }),
    ...
    execute: async (toolCallId, p) => {
      ...
      const remedy = managed
        ? 'Create or correct the file inside the managed project, for example under /workspace, then call WorkflowStart again.'
