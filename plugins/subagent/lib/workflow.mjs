...
const delegableAccess = (access) => ({
  ...
  // that adds nodes of its own unable to carry the account its whole workflow runs under.
...
export function registerWorkflow(ctx, getRun, { resolveDelegateTools, principalOf, dependencyContextChunks }) {
  ...
  const addNodesFromSession = (workflowId, rawNodes, callerSessionId, callerAccess, callerModel) => {
    ...
    for (const node of validatedNodes) {
      ...
      if (childAccess) {
        wf.nodeParentAccess.set(node.id, effectiveParentAccess);
    ...
    snapshot(wf);
    tick(wf);
    return { added: validatedNodes.map((node) => node.id) };
  };
  ...
  ctx.registerTool(defineTool({
    ...
    parameters: Type.Object({
      nodesFile: Type.String({ description: `Path to the JSON workflow definition. Create it with Write first — inside the project itself (for example the relative path workflow.json) in a managed project, otherwise under ${workflowDir} for a one-off run if your session may write there, else inside an accessible repository. Pass either a node array or { title?, fork?, nodes, background? }.` }),
      ...
      background: Type.Optional(Type.Boolean({ description: 'How this call delivers the workflow summary, overriding the file\'s own setting. Omitted (the default) and true both start the DAG asynchronously and deliver the summary in a NEW turn; false blocks until the whole DAG has finished and returns it inline. It changes delivery only — never the order in which nodes run.' })),
    }),
    execute: async (toolCallId, p) => {
      ...
      // The ambient execution target decides the route, never a refusal message. A managed project has no
      // host filesystem: the Write this tool tells the model to do first lands in the guest, and the host
      // path guard refuses every path there. Host turns keep the exact path guard they always had.
      const managed = ctx.currentAccess().projectRef?.kind === 'managed';
      // Where the caller should go and fix it, named for the filesystem it is actually standing on. A
      // managed caller pointed at "an accessible repository" would look for a host directory it has no
      // way to reach.
      const remedy = managed
        ? `Create or correct the file inside the managed project, for example under ${ctx.workDir() ?? 'the project directory'}, then call WorkflowStart again.`
        : 'Create or correct the file inside an accessible repository, then call WorkflowStart again.';
      let source;
      try {
        source = JSON.parse(managed
          ? await ctx.readManagedProjectFile(p.nodesFile)
          : readFileSync(ctx.assertPathAllowed(p.nodesFile), 'utf8'));
      } catch (e) {
        ...
        return ok(`Error: cannot read workflow file "${p.nodesFile}": ${message}. ${remedy}`);
      }
      let rawNodes;
      let fileOptions = {};
      ...
      const nodes = forkDefault
        ? validatedNodes.map((n) => (n.fork || n.tools || n.readOnly || n.subagentType ? n : { ...n, fork: true }))
        : validatedNodes;
  ...
  ctx.registerTool(defineTool({
    ...
    description: 'Re-run only the UNFINISHED nodes of a workflow that already stopped — one whose result named '
      ...
      + 'Behaves exactly like WorkflowStart otherwise — same asynchronous delivery, '
      + 'same live snapshot, same final summary — because it IS the same run continuing. Omit `background` '
      + 'and the resumed run keeps the delivery the run already had, which is asynchronous unless it was '
      + 'started with background=false.\n\n'
    ...
    parameters: Type.Object({
      ...
      background: Type.Optional(Type.Boolean({ description: 'How the resumed run delivers its summary. True delivers it in a NEW turn, false blocks until the remaining nodes finish. Omit to keep what this run already used — asynchronous delivery, unless the run was explicitly started with background=false.' })),
    }),
    execute: async (_id, p) => {
  ...
  ctx.registerTool(defineTool({
    ...
    execute: async (_id, p) => {
      ...
      // tool contract defines as success, so there is no case left to swallow.
      {
        ...
        const nodes = p.fork
          ? p.nodes.map((node) => (node.fork || node.tools || node.read_only || node.subagent_type
            ? node : { ...node, fork: true }))
          : p.nodes;
        let result;
