/** Canonical copy. `web/lib/codeDiff.ts` mirrors everything below byte for byte (Turbopack's root is
 *  pinned to `web/`, so the browser cannot import this file); `tests/contract/codeDiffMirror.test.ts`
 *  keeps the two identical. Edit this one first, then copy the body over. */
