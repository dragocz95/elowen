...
export function buildDigestPrompt(input: DigestInput): string {
  const lines: string[] = [
    ...
    '- "greeting": at most 6 words, a natural conversational opener addressed to the user (first name',
    '  is fine). NO time-of-day words like morning/evening — it is shown all day. No emoji, no',
    '  trailing punctuation. Do NOT copy a generic "good evening"; open the way YOU would, knowing them.',
    ...
    '  that clicking the button types into the chat, phrased as the user would ask it.',
    ...
    '- "suggestions": up to 3 concrete next steps continuing yesterday\'s unfinished threads. Never',
    '  duplicate a pill. "label" at most 5 words; "prompt" a complete ready-to-send instruction.',
