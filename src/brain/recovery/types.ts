 ...
 *
 *  Three substrates recover interrupted work at boot — durable delegation rows, the workflow engine's
 *  on-disk journal, and the park marker on an owner conversation's session row — and they are DELIBERATELY
 *  different: each stores, validates and terminalizes in the way its own failure mode demands. What they
 *  share is only the ORCHESTRATION around them: claim before the platforms open, resume after they are up,
 *  in a declared order, with one provider's failure never taking the others down.
 *
...
export interface RecoveryProvider<Item> {
   ...
   *  notice, refusing an unsafe replay — belongs to the provider, never to the coordinator. */
  resume(item: Item): Promise<void>;
}
