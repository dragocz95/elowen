...
export class LiveSessionSpawner {
  ...
  private async spawnOnce(opts: SpawnOpts): Promise<LiveBrain> {
    ...
    const { session, applyCompaction, assessColdCompaction } = await this.d.factory.create({
      ...
      // context handler at all. The cadence is a thunk deliberately: it is read per pass because the
      // operator may move the slider mid-conversation (do NOT latch it the way `toolDeferThreshold` is
      // latched above, which is a spawn-time routing decision). `providers` is a thunk only for symmetry
      // — it closes over THIS spawn's registry, which is the right answer because installing or toggling
      // a plugin goes through `reloadPlugins`, and that disposes every live session before the new
      // registry is used. `?? 0` is the unwired-dependency case — a process with no runtimeConfig —
      // which the seam treats as OFF, never as "every step".
