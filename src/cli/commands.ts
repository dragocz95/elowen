...
import { noTimeoutDispatcher } from './noTimeoutDispatcher.js';
...
export async function runApiCommand(
  ...
  // real answer, not undici's 300 s abort.
  const res = await deps.call(method, path, body, { url, token, dispatcher: noTimeoutDispatcher() });
