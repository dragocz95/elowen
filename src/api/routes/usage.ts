...
export function registerUsageRoutes(app: ElowenApp, ctx: RouteContext): void {
  ...
  app.post('/usage/reset', c => {
    ...
    // The origin rollup is an independent counter, so clearing only the message-derived side would leave
    // the admin drawer reporting spend the rest of Stats has just forgotten — the exact "the reset did
    // nothing" complaint the chat clear above was added to answer.
    const origins = userId != null ? d.usageOrigins?.clearForUser(userId) ?? 0 : 0;
    return c.json({ ok: true, cleared, chatCleared: chat, originsCleared: origins });
  });
}
