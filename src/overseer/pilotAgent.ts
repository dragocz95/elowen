...
export function pilotPrompt(goal: string, jobId: string, projectNotes?: string, cli: string = 'orca', models?: string, parallelism?: string): string {
  ...
  return render('pilot', { goal, notes, submit, jobId, models: models ?? '', parallelism: parallelism ?? '' });
}
...
export function makePilot(deps: { spawn: SpawnService; config: ConfigStore; projects: ProjectStore; planJobs: PlanJobStore; tmux: TmuxDriver; nameAgent: () => string; cli?: string }): (job: PlanJob, projectPath: string) => Promise<void> {
  return async (job, projectPath) => {
    ...
    const { session } = await deps.spawn.launch({
      ...
      rawPrompt: pilotPrompt(job.goal, job.id, notes, deps.cli, models, parallelism),
